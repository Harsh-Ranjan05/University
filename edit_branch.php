...
if(isset($_POST['submit'])){
    $branch = $_POST['branch'];
    $query = "UPDATE branch SET branch='$branch' WHERE id='$id'";
    $result = pg_query($conn, $query);
    if($result){
        echo "<script>alert('Updated Successfully'); window.location='branch.php';</script>";
    }else{
...
<main class="main-content">
  <header class="d-flex justify-content-between align-items-center mb-3">
    <h1 class="h4 fw-bold text-primary">Edit Branch</h1>
  ...
  <div class="card card-custom p-4 mb-4">
    <h5 class="mb-3">Update Branch Details</h5>
    <form method="POST">
      <div class="mb-3">
        <label class="form-label fw-semibold">Branch Name</label>
        <input type="text" name="branch" class="form-control" value="<?php echo $res['branch']; ?>" placeholder="Enter Branch Name" required>
      </div>
      ...
      while($res = pg_fetch_assoc($result)){ ?>
        <tr>
          ...
          <td>
            ...
            <a href="status.php?table=branch&id=<?php echo $res['id']; ?>" class="btn btn-sm <?php echo ($res['status']==1)?'btn-success':'btn-danger'; ?>">
              <?php echo ($res['status']==1)?'Active':'Deactive'; ?>
            </a>
          </td>
        </tr>
