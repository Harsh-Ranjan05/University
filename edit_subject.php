...
if(isset($_POST['submit'])){
    ...
    $query="UPDATE subject SET subject_name='$subject_name', subject_code='$subject_code' WHERE id='$id'";
    $result=pg_query($conn, $query);
    if($result){
...
<main class="main-content">
  <header class="d-flex justify-content-between align-items-center mb-3">
    <h1 class="h4 fw-bold text-primary">Edit Subject</h1>
  ...
  <div class="card card-custom p-4 mb-4">
    <h5 class="mb-3">Update Subject Details</h5>
    <form method="POST">
      <div class="mb-3">
        <label class="form-label fw-semibold">Subject Name</label>
        <input type="text" name="subject_name" class="form-control" value="<?php echo $subject_name; ?>" placeholder="Enter Subject Name" required>
      </div>
      <div class="mb-3">
        <label class="form-label fw-semibold">Subject Code</label>
        <input type="text" name="subject_code" class="form-control" value="<?php echo $subject_code; ?>" placeholder="Enter Subject Code" required>
      </div>
